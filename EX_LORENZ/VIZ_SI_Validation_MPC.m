...
legend_handle1 = legend(ph,'x1','x2','x3','Control','Location','NorthWest','location','NorthWest');
legend_handle1.Position = [legend_handle1.Position(1)+0.00 legend_handle1.Position(2)+0.03 legend_handle1.Position(3:4)];
...
legend_handle2.Position = [legend_handle1.Position(1)+0.27 legend_handle1.Position(2)+0.1 legend_handle2.Position(3:4)];

set(legend_handle2, 'Color', 'none');
% set(gca,'xlim',[0 tHistory(end)],'ylim',[-45 270],'Visible','off','Color','none')
set(gca,'LineWidth',1, 'FontSize',14)

print('-depsc2', '-cmyk', '-loose', [figpath,'EX_',SystemModel,'_MPC_',select_model,'_N_',num2str(N),'.eps']);

delete(legend_handle1)
delete(legend_handle2)
delete(th)
print('-depsc2', '-cmyk', '-loose', [figpath,'EX_',SystemModel,'_MPC_',select_model,'_clean','_N_',num2str(N),'.eps']);

set(ax1,'xlim',xlimval,'ylim',[ylimval(1),45])
set(ax2,'xlim',xlimval,'ylim',[ylimval(1),45],'Visible','off','Color','none')
set(gca,'xlim',xlimval,'ylim',[ylimval(1),45])
print('-depsc2', '-cmyk', '-loose',[figpath,'EX_',SystemModel,'_MPC_',select_model,'_clean_zoom','_N_',num2str(N),'.eps']);
...
set(gcf,'PaperPositionMode','auto')
print('-depsc2', '-cmyk', '-loose',[figpath,'EX_',SystemModel,'_MPC_',select_model,'_legend','_N_',num2str(N),'.eps']);
...
set(gcf,'Position',[100 100 450 100])
set(gcf,'PaperPositionMode','auto')
print('-depsc2', '-cmyk', '-loose',[figpath,'EX_',SystemModel,'_MPC_legend','.eps']);