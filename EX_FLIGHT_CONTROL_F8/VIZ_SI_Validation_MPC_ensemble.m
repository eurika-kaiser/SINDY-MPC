...
for iIC = 1:NicN
    ...
    ph(2) = plot([t_single;tA(2:end);tHistory(2:end)'],data2plot,'-','Color',ccolors(2,:),'LineWidth',1);
end
for iIC = 1:NicN
    data2plot = ([nan(size(t_single));xA(2:end,3);xHistory(3,2:end)']);
    data2plot(1:length(t)) = x(:,3,iIC);
    ph(3) = plot([t_single;tA(2:end);tHistory(2:end)'],data2plot,'-','Color',ccolors(3,:),'LineWidth',1);
end
for iIC = 1:NicN
    data2plot = ([nan(size(t_single));xA(2:end,1);xHistory(1,2:end)']);
    data2plot(1:length(t)) = x(:,1,iIC);
...
for iIC = 1:NicN
    ...
    u_data(1:length(t)) = u(:,1,iIC)+uval;
...
delete(th)
print('-depsc2', '-painters', '-cmyk', '-loose', [figpath,'EX_',SystemModel,'_MPC_',select_model,'_clean','_N_',num2str(N),'_ensemble.eps']);
...
print('-depsc2', '-painters', '-cmyk', '-loose',[figpath,'EX_',SystemModel,'_MPC_',select_model,'_clean_zoom','_N_',num2str(N),'_ensemble.eps']);
...
for jM = 1:Nmodels
    ph(jM) = plot([t_single;tA(2:end);tHistory(2:end)'],ones(length([t_single;tA(2:end);tHistory(2:end)']), 1),'-.','Color',ModelColors(2,:,jM),'LineWidth',1);
end
...
set(gcf,'PaperPositionMode','auto')
print('-depsc2', '-painters', '-cmyk', '-loose',[figpath,'EX_',SystemModel,'_MPC_',select_model,'_legend','_N_',num2str(N),'_ensemble.eps']);
...
figure; hold on, box on
ccolors = get(gca,'colororder');
ph(1) = plot([t_single;tA(2:end);tHistory(2:end)'],ones(length([t_single;tA(2:end);tHistory(2:end)']), 1),'-','Color',ccolors(1,:),'LineWidth',2);
ph(2) = plot([t_single;tA(2:end);tHistory(2:end)'],ones(length([t_single;tA(2:end);tHistory(2:end)']), 1),'-','Color',ccolors(2,:),'LineWidth',2);
ph(3) = plot([t_single;tA(2:end);tHistory(2:end)'],ones(length([t_single;tA(2:end);tHistory(2:end)']), 1),'-','Color',ccolors(3,:),'LineWidth',2);
ph(4) = plot([t_single;tA(2:end);tHistory(2:end)'],ones(length([t_single;tA(2:end);tHistory(2:end)']), 1),'-','Color','k','LineWidth',2);
set(gca,'xlim',[0 max(tHistory)],'ylim',[0 2])
...
set(gcf,'Position',[100 100 450 100])
set(gcf,'PaperPositionMode','auto')
print('-depsc2', '-painters', '-cmyk', '-loose',[figpath,'EX_',SystemModel,'_MPC_legend','_ensemble.eps']);